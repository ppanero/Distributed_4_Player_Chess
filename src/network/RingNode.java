...
public class RingNode implements Runnable {
    ...
    public static final String NETWORK_CONFIGURATION_FILENAME = "/src/network/netconfig.txt";
	...
	RingNode(int id){
		try {
			...
			socket = new DatagramSocket(netConfig.getPort());
	...
	public void sendMessage(){
		DataFrame frame = new DataFrame(destin, netConfig.getPort(), message);
	...
	public void sendFrame(DataFrame frame){
		...
		try{
			...
			packet = new DatagramPacket(buf, buf.length, 
					netConfig.getSuccessor(), netConfig.getPort());
			socket.send(packet);
	...
	public void makeCt(){
		DataFrame ctFrame = new DataFrame(netConfig.getPort());
		sendFrame(ctFrame);
	}
		...
		try {
			...
			while(receiving){
				...
				switch(checkFrame(frame)){
					...
					//purge ring
					case 2:
						if(frame.getSource() > netConfig.getPort()){
							sendFrame(frame);
						}else if(frame.getSource() == netConfig.getPort()){
							monitor = true;
						}else{frame.source_addr = netConfig.getPort(); sendFrame(frame);}
						break;
					...
					case 3:
						...
						if(!monitor ||
							...
							if(frame.getDes() == netConfig.getPort()){
